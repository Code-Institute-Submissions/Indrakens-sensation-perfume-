    ...
    if giftwrap:
        if item_id in list(shopping_bag.keys()):
            if giftwrap in shopping_bag[item_id]["items_by_giftwrap"].keys():
                ...
                messages.success(
                    request,
                    f'Updated {product.name.upper()} with {giftwrap.upper()}'
                    'quantity to'
                    f'{shopping_bag[item_id]["items_by_giftwrap"][giftwrap]}',
                )
            else:
                shopping_bag[item_id]["items_by_giftwrap"][giftwrap] = quantity
                messages.success(
                    request,
                    f"Added {product.name.upper()} with {giftwrap.upper()}"
        ...
        if item_id in list(shopping_bag.keys()):
            ...
            messages.success(
                request,
                f"Updated {product.name.upper()} quantity to"
                f"{shopping_bag[item_id]}",
            ) 
        else:
            shopping_bag[item_id] = quantity
            messages.success(
                request, f"Added {product.name.upper()} to your shopping bag"
            )
...
def adjust_shopping_bag(request, item_id):
    ...
    if giftwrap:
        if quantity > 0:
            ...
            messages.success(
                request,
                f'Updated {product.name.upper()}, {giftwrap.upper()} quantity'
                f'{shopping_bag[item_id]["items_by_giftwrap"][giftwrap]}',
            )
        else:
            ...
            messages.success(
                request,
                f"Removed {product.name.upper()} with included"
                f"{giftwrap.upper()} from shopping bag",
            )
    else:
        if quantity > 0:
            shopping_bag[item_id] = quantity
            ...
            shopping_bag.pop(item_id)
            messages.success(
                request, f"Removed {product.name.upper()} from shopping bag"
            )
...
def remove_from_shopping_bag(request, item_id):
    ...
    try:
        ...
        if giftwrap:
            ...
            messages.success(
                request,
                f"Removed {product.name.upper()} with {giftwrap.upper()}",
            )
        else:
            shopping_bag.pop(item_id)
            messages.success(
